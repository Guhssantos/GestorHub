...
.pulse-box{background:#F9FAFB;border-radius:12px;padding:16px 8px;
...
/* Esconde o date_input mas mantém funcional */
...
# ── BOTÃO MENU (components.v1.html = acesso real ao DOM pai) ─────────────────
...
#pill{
    ...
    padding:9px 18px 9px 14px;font-size:16px;
...
# Data vem do session_state (setado pelo calendário via st.date_input oculto)
...
# ── date_input OCULTO — controlado via JS ─────────────────────────────────
# O CSS acima o esconde visualmente, mas ele permanece funcional para o Streamlit
...
# Se o date_input foi alterado (pelo JS abaixo), atualiza session_state
if data_input != data_sel:
    st.session_state["data_agenda"] = data_input
    st.rerun()

# Botão sync invisível — acionado pelo JS do calendário
sync_clicked = st.button("sync_hidden", key="sync_btn", label_visibility="collapsed")
if sync_clicked:
    st.rerun()

# ── CALENDÁRIO VISUAL (components.v1.html) ────────────────────────────────
components.html(f"""
```
...
.cal-btn:hover{{border-color:#111827;box-shadow:0 2px 10px rgba(0,0,0,.12)}}
...
.sync-btn.spinning{{animation:spin .6s linear}}
...
border:1px solid #E5E7EB;padding:18px 16px;width:280px;
}}
...
padding:2px 8px;border-radius:8px;line-height:1;touch-action:manipulation}}
...
```
<button class="sync-btn" id="sync-btn" onclick="doSync()" title="Sincronizar">&#x21bb;</button>
...
function pick(dt){{
    ...
    closeCal();
    // Aciona o date_input nativo do Streamlit no documento pai
    setStreamlitDate(s);
}}

function setStreamlitDate(isoStr){{
    // Encontra o input de data oculto e dispara o evento de mudança
    var docs=[];
    try{{docs.push(window.parent.document)}}catch(e){{}}
    try{{if(window.top!==window.parent)docs.push(window.top.document)}}catch(e){{}}
    
    for(var i=0;i<docs.length;i++){{
        var inputs=docs[i].querySelectorAll('input[type="text"]');
        for(var j=0;j<inputs.length;j++){{
            var inp=inputs[j];
            // Streamlit usa inputs de texto mascarados para datas
            if(inp.closest('[data-testid="stDateInput"]')){{
                // Formata como DD/MM/YYYY para o Streamlit
                ...
                nativeInput.set.call(inp,formatted);
...
function doSync(){{
    ...
    // Aciona o botao sync oculto do Streamlit
    var docs=[];
    try{{docs.push(window.parent.document)}}catch(e){{}}
    try{{if(window.top!==window.parent)docs.push(window.top.document)}}catch(e){{}}
    for(var i=0;i<docs.length;i++){{
        // Procura botao com texto sync_hidden
        var btns=docs[i].querySelectorAll('button[kind="secondary"]');
        for(var j=0;j<btns.length;j++){{
            if(btns[j].textContent.includes("sync_hidden")){{
                btns[j].click();
                return;
            }}
        }}
        // Fallback: procura qualquer botao de rerun
        var stBtn=docs[i].querySelector('[data-testid="baseButton-secondary"]');
...
document.addEventListener("click",function(e){{
    var p=document.getElementById("popup"),t=document.getElementById("cal-toggle");
...
<p style="color:#6B7280;font-size:14px;margin-top:4px;font-family:Inter,sans-serif">Hoje, 10:00 • Duracao: 45m</p>
