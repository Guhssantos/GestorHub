...
[data-testid="stSidebar"] button {
    background: #7F1D1D !important; color: #FEE2E2 !important;
...
/* Esconde o date_input mantendo-o funcional */
div[data-testid="stDateInput"] {
    position: absolute !important; opacity: 0 !important;
    pointer-events: none !important; height: 0 !important; overflow: hidden !important;
}
/* Remove espaço extra dos iframes de components */
...
if opcao == "🏠 Inicio":
    ...
    # date_input oculto — acionado pelo JS do calendário
    ...
    # ── CALENDÁRIO — components.html com auto-resize ──────────────────────────
    # O iframe se redimensiona via JS: pequeno quando fechado, grande quando aberto.
    # Assim não há espaço em branco e o popup aparece completamente.
    components.html(f"""
<!DOCTYPE html>
<html>
<head>
...
#popup {{
    display:none; position:absolute; top:48px; left:0; z-index:999;
...
(function(){{
    ...
    var open    = false;
    var H_CLOSED = 52;   // altura do iframe quando popup fechado
    var H_OPEN   = 390;  // altura quando popup aberto

    function pad(n){{ return n<10?"0"+n:n; }}
    function iso(d){{ return d.getFullYear()+"-"+pad(d.getMonth()+1)+"-"+pad(d.getDate()); }}

    // Redimensiona o iframe no documento pai
    ...
    function render(){{
        ...
        for (var d=1; d<=days; d++){{
            ...
            add(g, d, false, dt, iso(dt)===iso(hoje), iso(dt)===iso(sel));
        }}
        var rem = (first+days)%7;
        if (rem) for (var i=1; i<=7-rem; i++) add(g, i, true, null);
    }}

    function add(g, txt, out, dt, isT, isS){{
    ...
    function pick(dt){{
        ...
        closeCal();
        sendDateToStreamlit(s);
    }}

    function sendDateToStreamlit(isoStr){{
        // Envia a data para o date_input oculto do Streamlit
    ...
    window.toggleCal = function(){{
        ...
        setHeight(open ? H_OPEN : H_CLOSED);
    ...
    window.doSync = function(){{
        ...
        // Força rerun do Streamlit sem navegar — re-dispara o date input
        var docs = [];
        try {{ docs.push(window.parent.document); }} catch(e) {{}}
        try {{ if(window.top!==window.parent) docs.push(window.top.document); }} catch(e) {{}}
        for (var i=0; i<docs.length; i++){{
    ...
    document.addEventListener("click", function(e){{
        ...
        if (open && p && t && !p.contains(e.target) && !t.contains(e.target)) closeCal();
    }});

    // Inicia com altura fechada
    setHeight(H_CLOSED);
}})();
</script>
</body>
